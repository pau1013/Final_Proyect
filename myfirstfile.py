...
class VentanaPrincipal(QtGui.QWidget):
    def __init__(self):
        ...
        self.connect(self.setup_Update_CPU_data, QtCore.SIGNAL('CPU_Data'),
                     self.graficaProceso)  # cambiar nombre de senal ####
        ###############################################
...
class Lista:
    ...
    def Ordenar(self, PID, CPU,
                MEM):  # Ordena la lista de menor a mayor de acuerdo a su pid,cpu,mem recibe boolean ejem(True,False,False)
    ...
    def busqueda_PID(self,
                     valor):  # Regresa el proc anterior al que se busco, la lista funciona solo con next, era necesario para remover
        temp = self.head
        if temp != None:
            if (temp.pid == valor):
                return temp
